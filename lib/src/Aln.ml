...
let cols seqs =
  if Array.length seqs = 0 then invalid_arg "DXMAF.Aln.cols"
...
let remove_vacuous_columns seqs =
  ...
  if not !any_col then failwith "DXMAF.Aln.stitch: all-gap alignment"
    ...
    | None ->
      ...
      with
        | StringTopSort.Unsat rows ->
          ...
          failwith (Printf.sprintf "DXMAF.Aln.stitch: two or more of the following rows are not in consistent order: %s" rows)
  let tot_conlen =
    List.fold_left (+) 0
      List.map
        fun (_,conseqs) ->
          let conlen = String.length conseqs.(0)
          Array.iter (fun s -> if String.length s <> conlen then failwith "DXMAF.Aln.stitch: found a block with inconsistent sequence lengths") conseqs
          conlen
        alns
